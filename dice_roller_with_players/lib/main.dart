...
class _DiceScreenState extends State<DiceScreen> {
  ...
  int player4Score = 0;
  String winnerMessage = "";
  bool _canRollAgain = false; // Indicates if the current player can roll again
  ...
  void _rollDice() {
    setState(() {
      if (totalRounds > 0) {
        _diceNumber1 = Random().nextInt(6) + 1; // Roll the dice (1-6)

        // Check if player rolls a 6
        if (_diceNumber1 == 6) {
          _addToPlayerScore();
          _canRollAgain = true; // Allow the player to roll again
        } else {
          _addToPlayerScore();
          _nextPlayer(); // Move to the next player if not 6
        }
      }
    ...
    return Row(
      ...
      children: [
        _buildDiceColumn(
          ...
          'Rolled Number: $_diceNumber1',
          'images/dice-$_diceNumber1.png',
          _canRollAgain ? _rollDice : null, // Roll again if player rolls 6
        ),
        SizedBox(width: 50),
        Column(
          children: [
  ...
  Widget _buildDiceColumn(
    ...
    return Column(
      children: [
        ...
        SizedBox(height: 10),
        ElevatedButton(
          onPressed: onPressed ?? _rollDice,
          child: Text(_canRollAgain ? 'Roll Again' : 'Roll the Dice'),
        ),
      ],
    );
  }
